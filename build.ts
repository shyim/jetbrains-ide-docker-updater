    ...
    for (let productKey of productKeys) {
        ...
        for (let release of json[productKey]) {
            ghConfig.matrix.include.push({
                ...
                runs: {
                    build: ` cd projector-docker; ./projector-docker/build-container.sh ghcr.io/shyim/jetbrains-ide/${productKey}:${release.version} ${release.downloads.linux.link}; docker push ghcr.io/shyim/jetbrains-ide/${productKey}:${release.version}`
                }
            });
        }
    }
